// src/pages/ChessBattleRanking.jsx
...
export default function ChessBattleRanking() {
    ...
    const run = async () => {
      ...
      try {
        ...
        const { data: list, error } = await supabase.from("game_scores")
          ...
          .eq("game_key", GAME_KEY).eq("level", LEVEL)
        ...
        setRows(top10);
        if (top10.length === 0) {
          setEmptyReason(myIsAdmin ? "관리자 계정은 랭킹에서 제외되어 표시되지 않아요." : "아직 대전 기록이 없어요. 첫 번째 승자가 되어보세요!");
        }
      } catch (e) {
        console.error("chess battle ranking load error:", e);
    ...
    <div className="ranking-page">
      <header className="top-header">
        <div className="top-row">
          <button type="button" className="ranking-nav-btn" onClick={() => navigate("/chess-battle")}>체스대전으로</button>
          <h1 className="app-title">체스대전 랭킹</h1>
      ...
      {loading ? (
        ...
        <div className="ranking-section">
          <div className="ranking-section-title">체스 대전</div>
      ...
      <div className="ranking-tip ranking-tip-spaced">누적 승수 기준으로 순위가 매겨져요.</div>
      <div className="ranking-tip ranking-tip-spaced">
        <button type="button" className="hanja-btn ghost ranking-back-btn" onClick={() => navigate("/chess-battle")}>체스대전으로 돌아가기</button>
      </div>
    </div>
  );
}
