...
const SECTIONS = [
  { key: "8", title: "8급", level: "8" },
  { key: "7", title: "7급", level: "7" },
  { key: "6", title: "6급", level: "6" },
];
...
export default function HanjaRanking() {
  ...
  // 급수별 Top10을 한 번에 들고 있기
  const [rowsByLevel, setRowsByLevel] = useState({}); // { "8": [...], "7": [...], "6": [...] }

  // 급수별 내 최고점
  const [myBestByLevel, setMyBestByLevel] = useState({}); // { "8": number|null, ... }
    ...
    const run = async () => {
      ...
      try {
        ...
        const fetchTop10 = async (level) => {
          ...
          // 관리자 계정 제외를 위해 프로필에서 is_admin을 모아서 확인
        ...
        console.error("hanja ranking load error:", e);
        setRowsByLevel({});
        setMyBestByLevel({});
