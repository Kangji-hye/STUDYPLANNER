...
function getSoundLabelByValue(value) {
  ...
  const found = FINISH_SOUNDS.find((x) => x.value === v);
  return found?.label || "기본";
}
...
const MyPage = () => {
  ...
  const [alarmPermission, setAlarmPermission] = useState("default");
  ...
  const loadMyProfile = async () => {
    ...
    const baseProfile = profileError
      ...
      : {
          ...
          grade_code: Number.isFinite(Number(profileData?.grade_code))
            ? Number(profileData.grade_code)
            : GRADE_OTHER,
          grade_manual: Boolean(profileData?.grade_manual),
          alarm_enabled:
            typeof profileData?.alarm_enabled === "boolean" ? Boolean(profileData.alarm_enabled) : null,
        };
  ...
  const turnAlarmOn = async () => {
    ...
    if (!("Notification" in window)) {
      alert(
        "이 기기/브라우저에서는 알림이 지원되지 않을 수 있어요.\n\n" +
          "가능하면 홈 화면에 추가한 뒤 앱처럼 실행해 주세요."
      );
      return;
    }
    ...
    if (p === "granted") {
      ...
      alert("알림이 차단되어 있어요. 기기/브라우저 설정에서 알림을 허용해 주세요.");
    }
  };
    ...
    <div className="mypage">
      ...
      <div className="level-panel">
        ...
        <div className="level-sub">
          달력에 도장이 현재 {stampCount}개, 다음 레벨까지 {levelInfo.stampsToNext}개 남았어요
        </div>
          ...
          <span className="value mypage-sound">
            <div className="sound-card">
              <div className="sound-card-head">
                ...
                <span className="sound-hint">마지막 완료를 눌렀을 때 이 소리가 나와요</span>
              </div>
